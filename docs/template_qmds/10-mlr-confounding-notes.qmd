...
-   Download [10-mlr-confounding-notes.qmd](template_qmds/10-mlr-confounding-notes.qmd), open it in RStudio, and save it in the “Activities” sub-folder of the “STAT 155” folder.

#### If you're early

-   Pick up your nametag and the others in the stack you'll be sitting with, choose a table! (please don't scatter them 🥺)
...
-   Thursday: Quiz 1 revisions due at the beginning of class
...
In many statistical models (especially those we're learning in this course), we are often just interested in two variables:

-   Y = the response variable or outcome of interest

-   X = the predictor of interest

In these models, we are often trying to establish evidence that there is a **causal** relationship between X and Y

-   NOTE THAT REGRESSION MODELS CANNOT "PROVE" CAUSALITY, ONLY PROVIDE EVIDENCE TO SUGGEST THAT THERE MIGHT BE A CAUSAL PATHWAY FROM X TO Y

When exploring the relationship of Y with X, we almost always want to consider other variables (Z) that also have a causal or correlative relationship with Y and/or X. The nature of those relationships helps us determine whether to include or exclude Z as a predictor in the model.

The first type of variable we will introduce is called a **confounder**, which we **always** want to include in our model.

-   Z is a **confounder** if…

    -   it is *causally* associated with both Y and X; **or**

    -   it is *causally* associated with Y, and just *associated* (not causally) with X

-   **Why do we want to include Z in our model?**

    -   because failing to "adjust for" (or "control for") Z leads to an incorrect conclusion about the relationship between Y and X.

# Warm-up

## Exercise 0

Think back to the model we looked at on Tuesday, where we were interested in the effect of wind speed (X) on number of rides (Y). Compare the output from a model without the temp_feel (Z) variable (`bike_model_a`), and one with the temp_feel variable as a predictor (`bike_model_b`).

**What happens to the coefficient for the `windspeed`** **variable when we include `temp_feel`**? Why?

**What happens to the** $R^2$ value when we include `temp_feel`? Why?

```{r}
library(tidyverse) 
bikes <- read_csv("https://mac-stat.github.io/data/bikeshare.csv") %>% rename(rides = riders_registered)  

bike_model_a <- lm(rides ~ windspeed, data = bikes)
summary(bike_model_a)
```

```{r}
bike_model_b <- lm(rides ~ windspeed + temp_feel, data = bikes)
summary(bike_model_b)
```
